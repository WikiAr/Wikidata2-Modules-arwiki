...
function p.list()
    local TABLE_HEADERS = { "ns_10_name", "template_result", "link", "name", "redirects" }

    local function createTableCell(row, content)
        return row:tag("td"):tag("span"):wikitext(content)
    end

    local function formatWikiLink(prefix, value)
        return string.format("[[%s %s]]", prefix, value)
    end

    local function buildRedirectsCell(row, language_codes, main_code)
        local cell = row:tag("td"):attr("dir", "ltr")

        local codes = {}
        if language_codes then
            for i, value in ipairs(language_codes) do
                codes[i] = value
            end
        end

        codes[#codes + 1] = main_code

        for i, code in ipairs(codes) do
            if isvalid(code) then
                cell:tag("code"):wikitext(code)
                if i < #codes then
                    cell:tag("span"):wikitext(" - ")
                end
            end
        end
        return cell
    ...
    local tableBuilder = mw.html.create("table")
        :addClass("wikitable sortable collapsible mw-collapsed")


    local headerRow = tableBuilder:tag("tr")
    for _, key in ipairs(TABLE_HEADERS) do
        headerRow:tag("th"):wikitext(i18n.list[key])
    end

    local frame = mw.getCurrentFrame()
    for code, language_data in pairs(data.lang_table) do
        local row = tableBuilder:tag("tr")

        local templateName = i18n.list.template_ISO_639_name
        local templateLink = formatWikiLink(i18n.ns_10_name .. ":" .. templateName, code)
        local templateResult = frame:preprocess(string.format("{{%s %s}}", templateName, code))
        local simpleLangLink = formatWikiLink(i18n.list.lang_name_prefix, language_data.name)
        local fullLangLink = formatWikiLink(
            i18n.list.lang_name_prefix_with_the,
            gsubname(language_data.name, "r", code)
        )

        createTableCell(row, templateLink)
        createTableCell(row, templateResult)
        createTableCell(row, simpleLangLink)
        createTableCell(row, fullLangLink)
        buildRedirectsCell(row, language_data.codes, code)
    end

    return tostring(tableBuilder)
end
